    ...
    <div class="flex pb-12 w-full">
        ...
        <div class="flex justify-center w-7/12">
            <div class="w-full p-6 rounded-lg" style="font-family:Verdana;background-color:var(--div-dark);color:var(--jay-blue);">
                ...
                <form id="cancel" action="{{ route('game', [$game->user()->username, $game->name]) }}" enctype="multipart/form-data" method="get">@csrf</form>
                <form id="delete" action="{{ route('delete_game', [$game->user()->username, $game->name]) }}" enctype="multipart/form-data" method="post">@csrf</form>
                <form id="form" action="{{ route('edit_game', [$game->user()->username, $game->name]) }}" enctype="multipart/form-data" method="post" onkeydown="return event.key != 'Enter';">
                    @csrf
                    <div class="mb-4">
                        Name:
                        ...
                        <input type="text" maxlength="60" originalvalue="{{$game->name}}" id="name" onclick="selectText(this)" onfocusout="resetValue(this)" name="name" id="name" placeholder="{{ $game->name }}" class="nice-input-1 nice-edit-1" spellcheck="false" value="{{ $game->name }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" maxlength="150" name="simple_description" originalvalue="{{$game->simple_description}}" id="simple_description" onfocusout="resetValue(this)" onclick="selectText(this)" placeholder="{{ $game->simple_description }}" class="nice-input-1 nice-edit-1" value="{{ $game->simple_description }}">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="detailed_description" class="nice-input-1 nice-edit-1" onfocusout="resetValue(this)" onclick="selectText(this)" style="text-align:left" id="detailed_description" cols="30" rows="4" originalvalue="{{$game->detailed_description}}" placeholder="{{ $game->detailed_description }}">{{ $game->detailed_description }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <button type="button" class="nice-input-1" style="margin-top:10px;padding:10px;padding-right:16px;padding-left:16px;border-radius:5px;width:auto;display:inline;"name="game_button" onclick="clickFileButton()" original-zip="{{ substr(str_replace(" ","_",$game->name) ."_". $game->user()->username, 0, 18) }}.zip" id="game_button">{{ substr(str_replace(" ","_",$game->name) ."_". $game->user()->username, 0, 18) }}.zip</button>
                    ...
                    <div class="mb-4">
                        ...
                        <div style="margin-top:3px">
                            <input type="number" min="0" max="4096" originalvalue="{{$game->width}}" onfocusout="resetValue(this)" name="width" id="width" placeholder="{{ $game->width }}px" class="nice-input-1 nice-edit-1" style="width:49.6%;" value="{{ $game->width }}">

                            <label for="height" class="sr-only"></label>
                            <input type="number" min="0" max="4096" originalvalue="{{$game->height}}" onfocusout="resetValue(this)" name="height" id="height" placeholder="{{ $game->height }}px" class="nice-input-1 nice-edit-1" style="width:49.6%;" value="{{ $game->height }}">
                    ...
                    <div>
                        <label for="fullscreen" class="cool-checkbox-container" style="width:250px">Allow Full Screen?
                            <input type="checkbox" name="fullscreen" id="fullscreen" {{ $game->fullscreen ? 'checked' : '' }}>
                ...
                <button type="cancel" form="cancel" class="form-action-button form-cancel-button half-width-3">Cancel</button>
                <button onclick="confirmDelete()" class="form-action-button form-delete-button half-width-3">Delete</button>
            </div>
        </div>
